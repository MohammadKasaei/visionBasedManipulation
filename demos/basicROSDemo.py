...
class GraspControl():
    ...
    def __init__(self,env,network_model):
        ...
        rospy.init_node("basic_ros_demo")
...
0]
    ...
    def generatePointcloud(self, depth, rgb, parent_frame):
        ...
        fields = [sensor_msgs.msg.PointField(
            ...
            for i, n in enumerate(pc_fields)]
        ...
        msg = sensor_msgs.msg.PointCloud2(
            ...
            is_bigendian=False,
            fields=fields,
            point_step=(itemsize * len(pc_fields)),
            row_step=(itemsize * len(pc_fields) * pc.shape[0]),
            data=data
        )

        self.publishPointCloud(msg)

    
    def publishPointCloud(self, pc_msg):
    ...
    def graspStateMachine(self):
        ...
        if  self.gState  == "Stop":
            ...
            number_of_predict = 1
            output = False
            self.generatePointcloud(depth, rgb, "camera_depth_optical_frame")
