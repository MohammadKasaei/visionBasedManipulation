...
from graspControl.graspControl import GraspControl
# class GraspControl():
...
if __name__ == '__main__':
    ...
    env = BaiscEnvironment(GUI = True,robotType ="UR5",img_size= 544)
    number_of_objects = 5
    env.updateBackgroundImage(1)
    env.objects.shuffle_objects()
    info = env.objects.get_n_first_obj_info(number_of_objects)
    # info = [
    #         ("objects/ycb_objects/YcbBanana/model.urdf", False, False)
    #     ]
    ...
    import cv2
    img, dep = env.captureImage(0)
    dep = (dep * 1000).astype(np.uint8)
    print(np.max(dep), np.min(dep), np.mean(dep))
    cv2.imwrite("/home/user/Documents/visionBasedManipulation/datas/rgb_3.png", img)
    cv2.imwrite("/home/user/Documents/visionBasedManipulation/datas/dep_3.png", dep)
    
    # gc = GraspControl(env,networkName) 
