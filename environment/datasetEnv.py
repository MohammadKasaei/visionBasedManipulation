...
from environment.camera.camera import Camera, VGNCamera, CameraIntrinsic
import math
import time
import numpy as np
...
class DatasetEnvironment:
    ...
    def updateBackgroundImage(self,stage):
        ...
        if (stage == 0):
            self.bgBGRWithoutBox, self.bgDepthWithoutBox, self.bgSegWithoutBox = self.camera.get_cam_img()
        elif (stage == 1):
            self.bgBGRBox, self.bgDepthBox, self.bgSegBox = self.camera.get_cam_img()
            

    def captureImage(self,removeBackground=0, getSegment=False): 
        bgr, depth, seg = self.camera.get_cam_img()
        
        if (removeBackground):                      
           bgr = bgr-self.bgBGRBox+self.bgBGRWithoutBox
           depth = depth-self.bgDepthBox+self.bgDepthWithoutBox
           seg = seg-self.bgSegBox+self.bgSegWithoutBox
