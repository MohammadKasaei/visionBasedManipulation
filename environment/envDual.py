...
class Environment:
    ...
    TARGET_ZONE_POS = [0.7, -0.5, 0.685]
    SIMULATION_STEP_DELAY = 0.0005 #speed of simulator - the lower the fatser
    FINGER_LENGTH = 0.04 #0.06
    Z_TABLE_TOP = 0.785
    GRIP_REDUCTION = 0.3

    def __init__(self, camera: Camera, vis=False, debug=False, gripper_type='85', finger_length=0.02) -> None:
        self.vis = vis
        self.debug = debug
        self.camera = camera
        ...
        self.UR5Stand_id = p.loadURDF('environment/urdf/objects/ur5_stand.urdf',
                                      ...
                                      p.getQuaternionFromEuler([0, 0, 0]),
                                      useFixedBase=True)
        self.robot1_id = p.loadURDF('environment/urdf/ur5_robotiq_%s.urdf' % gripper_type,
                                   [0, 0, 0.0],  # StartPosition
                                   p.getQuaternionFromEuler([0, 0, 0]),  # StartOrientation
                                   useFixedBase=True,
                                   flags=p.URDF_USE_INERTIA_FROM_FILE)
        self.joints, self.controlGripper1, self.controlJoints, self.mimicParentName =\
            setup_sisbot(p, self.robot1_id, gripper_type)
        ...
        p.enableJointForceTorqueSensor(
            self.robot1_id, self.joints['left_inner_finger_pad_joint'].id)
        p.enableJointForceTorqueSensor(
            self.robot1_id, self.joints['right_inner_finger_pad_joint'].id)

        # Change the friction of the gripper
        p.changeDynamics(
            self.robot1_id, self.joints['left_inner_finger_pad_joint'].id, lateralFriction=1)
        p.changeDynamics(
            self.robot1_id, self.joints['right_inner_finger_pad_joint'].id, lateralFriction=1)
        
        
        ###
        self.UR5Stand_id = p.loadURDF('environment/urdf/dual_franka.urdf',
                                      [-0.7, -0.36-1.2, 0.1],
                                      p.getQuaternionFromEuler([0, 0, 0]),
                                      useFixedBase=True)
        ...
        self.robot2_id = p.loadURDF('environment/urdf/ur5_robotiq_%s.urdf' % gripper_type,
                                   [0, -1.2, 0.0],  # StartPosition
                                   p.getQuaternionFromEuler([0, 0, 0]),  # StartOrientation
                                   useFixedBase=True,
                                   flags=p.URDF_USE_INERTIA_FROM_FILE)

        self.joints2, self.controlGripper2, self.controlJoints2, self.mimicParentName2 =\
            setup_sisbot(p, self.robot2_id, gripper_type)

        p.enableJointForceTorqueSensor(
            self.robot2_id, self.joints2['left_inner_finger_pad_joint'].id)
        p.enableJointForceTorqueSensor(
            self.robot2_id, self.joints2['right_inner_finger_pad_joint'].id)

        # Change the friction of the gripper
        p.changeDynamics(
            self.robot2_id, self.joints2['left_inner_finger_pad_joint'].id, lateralFriction=1)
        p.changeDynamics(
            self.robot2_id, self.joints2['right_inner_finger_pad_joint'].id, lateralFriction=1)
        ...
        self.ee_position_limit = ((-0.8, 0.8),
                                  (-0.8, 0.8),
                                  (0.785, 1.4))
        self.robot_id = self.robot1_id
        self.controlGripper = self.controlGripper1
        self.reset_robot()

        self.robot_id = self.robot2_id
        self.controlGripper = self.controlGripper2
        self.reset_robot()
    ...
    def go_home_pos(self,robotNo):
        y_orn = p.getQuaternionFromEuler([-np.pi*0.25, np.pi/2, 0.0])
        if (robotNo == 1):
            self.move_ee([0.7*self.TARGET_ZONE_POS[0],
                    0*self.TARGET_ZONE_POS[1], 1.2, y_orn])
        else:
            self.move_ee([self.TARGET_ZONE_POS[0]*0.7-0.5,
                        -1+self.TARGET_ZONE_POS[1], 1.2, y_orn])
    ...
    def create_pile(self, obj_info):
        box_ids = self.create_temp_box(0.35, 1)
    ...
    def move_ee(self, action, max_step=300, check_collision_config=None, custom_velocity=None,
        ...
        for _ in range(max_step):
            # apply IK
            ee   = p.getLinkState(self.robot_id,self.eef_id)[0]
            xc,yc,zc = 0.9*np.array(ee)+0.1*np.array([x,y,z])
