...
def predict(tsdf_vol, net, device):
    ...
    with torch.no_grad():
        ...
        print(qual_vol.shape)
        print(rot_vol.shape)
        print(width_vol.shape)
...
def process(
    ...
    outside_voxels = tsdf_vol > 1.0
    inside_voxels = np.logical_and(1e-3 < tsdf_vol, tsdf_vol < 1.0)
    valid_voxels = ndimage.morphology.binary_dilation(
        outside_voxels, iterations=2, mask=np.logical_not(inside_voxels)
    )
    qual_vol[valid_voxels == False] = 0.0

    print(np.sum(qual_vol))
...
def select(qual_vol, rot_vol, width_vol, threshold=0.0, max_filter_size=4):
